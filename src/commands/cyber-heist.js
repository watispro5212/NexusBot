    ...
    async execute(interaction) {
        ...
        if (user.balance < 500) {
            return interaction.reply({ 
                embeds: [createEmbed({
                    title: 'Access Denied',
                    description: 'You need at least \`500\` Credits to bypass the initial firewall uplink.',
                    type: 'error'
                })],
                flags: 64
            });
        ...
        collector.on('collect', async i => {
            if (i.user.id !== interaction.user.id) return i.reply({ content: 'Unauthorized access.', flags: 64 });
